...
for sc in spec["scenes"]:
    ...
    for ln in sc["lines"]:
        ...
        text = ln.get("cap", ln["text"])
        s, sr = k.create(ln["text"], voice=ln.get("voice", spec["voice"]), speed=spec["speed"], lang=ln.get("lang", "en-us"), is_phonemes=ln.get("phonemes", False))
        assert sr == SR
...
json.dump({"total": round(t, 3), "scenes": scenes}, open("timeline.json", "w"), indent=1, ensure_ascii=False)
